...
function renderServices(lang){
  ...
  el.innerHTML = SERVICES.map((s,i)=>{
    ...
    const media = shots.length
      ? `<div class="service-shots n${n}">${shots.slice(0,4).map(p=>`<figure class="shot"><img src="assets/img/${p}?v=36" alt="${t('services.'+s.key+'.title',lang)} — ${t('real.tag',lang)}" loading="lazy"></figure>`).join('')}</div>`
