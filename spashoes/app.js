...
const SERVICES = [
  ...
  { key:'dye', ico:'brush', ba:true, photos:['real/dye-beige.jpg'],
...
function renderServices(lang){
  ...
  el.innerHTML = SERVICES.map((s,i)=>{
    ...
    const shots = s.photos.map((p,pi)=>{
      ...
      return `<figure class="sc-shot${s.ba?' is-ba':''}${fill}"${ar}><img src="assets/img/${p}?v=54" alt="${title}" loading="lazy">${s.ba?`<span class="sc-mini-tag">${before} · ${after}</span>`:''}</figure>`;
    }).join('');
    return `
...
function openServiceModal(key, sourceCard){
  ...
  modal.querySelector('.svm-ba-img').src='assets/img/'+s.ba+'?v=54';
  ...
  gal.innerHTML=(s.gallery||[]).map(p=>`<figure class="svm-tile"><img src="assets/img/${p}?v=54" alt="${t('real.tag',lang)}" loading="lazy"></figure>`).join('');
